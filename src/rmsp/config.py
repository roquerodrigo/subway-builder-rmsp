...
def _default_use_docker() -> bool:
    """Docker on Linux (no Homebrew); native binaries on macOS. ``RMSP_USE_DOCKER``
    (1/0, true/false, yes/no) overrides either way."""
    env = os.environ.get("RMSP_USE_DOCKER")
    if env is not None:
...
class Settings:
    ...
    description: str = (
        "A maior metrópole do hemisfério sul. Região Metropolitana de São Paulo "
        "inteira — capital, ABC, Guarulhos, Osasco/Barueri, Mogi e além — com "
        "~20 mi de habitantes. Demanda baseada na Pesquisa Origem-Destino do Metrô-SP."
    )
    ...
    # still lands inside the ~300 m sub-cells, so keep all by default.
    ...
    # Default: Docker on Linux, native Homebrew binaries on macOS (RMSP_USE_DOCKER overrides).
    use_docker: bool = field(default_factory=_default_use_docker)
