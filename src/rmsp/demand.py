...
The official OD survey fixes the per-zone totals and the zone→zone matrix; the
sub-point proxy (:mod:`rmsp.subpoints`) only decides *where inside a zone* demand
...
Two bases (``settings.residents_basis``), both respecting the official survey:
  workers  – FE_PESS per person, home zone (ZONA) → workplace zone (ZONATRA1);
             totals = the working population (~10.4M).
