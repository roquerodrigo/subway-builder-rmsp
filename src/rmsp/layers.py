...
# pedestrian highway types — only kept where they fall inside a park (park trails),
# otherwise the city floods with sidewalks/crossings. They classify as "minor".
_PATH = frozenset({"footway", "path", "pedestrian", "cycleway", "steps"})


def _parks_index():
    """STRtree of park polygons, to keep only the pedestrian paths that touch a park."""
...
def build_roads() -> None:
    ...
    for ft in geojson.read_features(settings.sources_dir / "roads.geojsonseq"):
        ...
        # drop pedestrian paths that aren't inside any park
        if (props.get("highway") or "") in _PATH and not len(
            parks.query(LineString(coords), predicate="intersects")
        ):
            continue
        feats.append(
            {
...
def _buildings_chunk(path, start: int, end: int) -> list[dict]:
    """Process one byte-range of buildings.geojsonseq into game building dicts.

    Pure per-feature work (bbox cull, area cull, simplify, round) with no cross-feature
    state, so chunks run in parallel and concatenate losslessly; the global grid is
    built once in :func:`build_buildings`. Top-level for ProcessPoolExecutor pickling."""
    ...
    for ft in geojson.read_features_range(path, start, end):
        ...
        for ring in geojson.outer_rings(geom):
            ...
            out.append({"b": [round(v, dec) for v in bx], "f": f, "p": p})
    return out
...
def build_buildings() -> None:
    ...
    src = settings.sources_dir / "buildings.geojsonseq"
    buildings: list[dict] = [
        b
        for part in geojson.parallel_chunks(src, _buildings_chunk, settings.n_build_workers())
        for b in part
    ]
    if not buildings:
        raise RuntimeError("no buildings in bbox")
    minx = min(b["b"][0] for b in buildings)
    miny = min(b["b"][1] for b in buildings)
    maxx = max(b["b"][2] for b in buildings)
    maxy = max(b["b"][3] for b in buildings)
