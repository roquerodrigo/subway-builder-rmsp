"""Disaggregate per-zone OD demand into sub-points within each zone (dasymetric).

The official OD survey fixes how *much* residential/job demand each OD zone has;
this module decides *where inside the zone* it sits. A configurable proxy
(``settings.demand_proxy``) supplies the weighted points: CNEFE 2022 address
points (default — residences weighted by real census setor population, jobs by
establishment density) or OSM building footprints (area × levels split by tags).
Points are bucketed into a grid within their zone, and each non-empty cell
becomes one demand sub-point at its weighted centroid. Nothing here alters
official magnitudes.
"""
...
# pyshp logs a warning for every OD-zone shape with orphaned interior rings while
# converting to GeoJSON — harmless for our point-in-polygon use, just noise. Silence it
# at module import so it's quiet in the main process and in every worker (which re-import).
